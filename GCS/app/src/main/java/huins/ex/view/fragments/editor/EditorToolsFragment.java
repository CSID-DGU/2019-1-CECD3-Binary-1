package huins.ex.view.fragments.editor;
...
import huins.ex.model.adapters.AdapterMissionItems;
...
public class EditorToolsFragment extends ApiListenerFragment implements OnClickListener {
    ...
    public void onApiConnected() {
        ...
        final ImageButton buttonSave = (ImageButton) getView().findViewById(R.id.editor_tools_marker_save);
        ...
        final ImageButton buttonHistory = (ImageButton) getView().findViewById(R.id.editor_tools_marker_history);
    ...
    private void saveMissionFile() {
        ...
        dialog.show(getFragmentManager(), MISSION_FILENAME_DIALOG_TAG);
    }
    ...
    public void onSaveInstanceState(Bundle savedInstanceState) {
        ...
        savedInstanceState.putString(EXTRA_OPENED_MISSION_FILENAME, openedMissionFilename);
    ...
    public void onClick(View view) {
        EditorTools newTool = getToolForView(view.getId());
        final int xOff = (int) (view.getWidth() + popupLeftMargin);
        final int yOff = -view.getHeight();
        switch (newTool) {
            case SELECTOR:
                selectorPopup.showAsDropDown(view, xOff, yOff);
                break;

            case TRASH:
                trashPopup.showAsDropDown(view, xOff, yOff);
                break;

            case DRAW:
                drawPopup.showAsDropDown(view, xOff, yOff);
                break;

            case MARKER:
                markerPopup.showAsDropDown(view, xOff, yOff);
                break;
        }
        setTool(newTool);
    }
    ...
    private static class DrawToolsImpl extends EditorToolsImpl implements AdapterView.OnItemSelectedListener {
        ...
        @Override
        public void onPathFinished(List<Coordinate> points) {
            if (missionProxy != null) {
                switch (selectedType) {
                    case WAYPOINT:
                    default:

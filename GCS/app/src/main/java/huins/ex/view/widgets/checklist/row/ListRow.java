...
public class ListRow implements ListRow_Interface, OnClickListener, OnLongClickListener {
	...
	protected void getFlightVariable(Flight mFlight, CheckListItem mListItem) {
		String sys_tag = mListItem.getSys_tag();

        final Battery flightBattery = mFlight.getAttribute(AttributeType.BATTERY);
		if (sys_tag.equalsIgnoreCase("SYS_BATTREM_LVL")) {
			mListItem.setSys_value(flightBattery.getBatteryRemain());
		} else if (sys_tag.equalsIgnoreCase("SYS_BATTVOL_LVL")) {
			mListItem.setSys_value(flightBattery.getBatteryVoltage());
		} else if (sys_tag.equalsIgnoreCase("SYS_BATTCUR_LVL")) {
			mListItem.setSys_value(flightBattery.getBatteryCurrent());
		}

        final Gps flightGps = mFlight.getAttribute(AttributeType.GPS);
        if (sys_tag.equalsIgnoreCase("SYS_GPS3D_LVL")) {
			mListItem.setSys_value(flightGps.getSatellitesCount());
		}

        final State flightState = mFlight.getAttribute(AttributeType.STATE);
        if (sys_tag.equalsIgnoreCase("SYS_ARM_STATE")) {
			mListItem.setSys_activated(flightState.isArmed());
		} else if (sys_tag.equalsIgnoreCase("SYS_FAILSAFE_STATE")) {
			mListItem.setSys_activated(flightState.isWarning());
		} else if (sys_tag.equalsIgnoreCase("SYS_CONNECTION_STATE")) {
			mListItem.setSys_activated(flightState.isConnected());
		}
	}

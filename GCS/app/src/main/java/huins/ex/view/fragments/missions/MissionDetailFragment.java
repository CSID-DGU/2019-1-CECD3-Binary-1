...
import huins.ex.view.fragments.helper.ApiListenerDialogFragment;
            ...
            for (MissionItemProxy missionItemProxy : mSelectedProxies) {
                ...
                if (previousType != selectedType) {
                    ...
                    if (previousType == MissionItemType.SURVEY) {
                        ...
                        final List<Coordinate> polygonPoints = previousSurvey.getPolygonPoints();
                        for (Coordinate coordinate : polygonPoints) {
                            final MissionItem newItem = selectedType.getNewItem();
                            if (newItem instanceof MissionItem.SpatialItem) {
                                ((MissionItem.SpatialItem) newItem).setCoordinate(new CoordinateExtend(coordinate
